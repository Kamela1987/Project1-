...
| Live location / matching cache | **Redis** | Sub-second read/write for "where are all online drivers right now," trip-state pub/sub, and the request-matching queue. |
| Real-time transport | **WebSocket (Socket.IO)**, with SMS fallback | Push live driver location and trip status to both apps; when a device drops off data, fall back to SMS for critical state changes (trip accepted, driver arrived, trip completed). |
| Admin dashboard | **React + TypeScript + Tailwind/shadcn** | Fast to build CRUD-heavy ops screens; same API as the mobile apps. |
| Payments | **MTN Mobile Money & Airtel Money APIs** (primary), cash (fallback, tracked in-app), card (future/optional via a gateway like Flutter wave/DPO if ever needed) | Matches how money actually moves in Monze today. |
...
- **Commission model:** implemented in the Phase 1 scaffold as a per-vehicle-type percentage (15% car / 12% minibus / 10% motorbike — see [`backend/src/config/commission.config.ts`](../backend/src/config/commission.config.ts)) deducted into a driver `Wallet` as a debt, since cash trips mean the driver already holds the fare; a driver owing more than K100 is blocked from going online until it's settled (`POST /drivers/:driverId/wallet/settlements`). Open for stakeholders to confirm: are these the right rates, is K100 the right block-online threshold, and should settlement eventually happen via mobile money instead of admin-recorded cash?
